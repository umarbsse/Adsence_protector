...
?>
<?php
    function clicks_setting(){
        if (isset($_GET['action']) && $_GET['action']!="" && $_GET['action']=="clear_records") {
            global $wpdb;
            $plugin_table_prefix = "ads_prtctr_";
            $table = $wpdb->prefix . $plugin_table_prefix.'clicks';
            $delete = $wpdb->query("TRUNCATE TABLE $table");
            wp_redirect(admin_url().'admin.php?page=adsense-protector-clicks');
        }
    }
?>
<?php
	function clicks_html(){
        ...
        $sql = " SELECT $clicks_tbl.*,$ip_detail_tbl.*,wp_ads_prtctr_bann_ip.*,$ip_detail_tbl.ip as real_ip,$bann_countries_tbl.icon ";
        $sql.= " FROM `$clicks_tbl` ";
        $sql.= " LEFT JOIN $ip_detail_tbl ON $ip_detail_tbl.id=$clicks_tbl.ip";
        $sql.= " LEFT JOIN $bann_countries_tbl on $bann_countries_tbl.country_name=$ip_detail_tbl.country  ";
        $sql.= " LEFT JOIN $bann_ip_tbl  ";
        $sql.= " ON $bann_ip_tbl.ip=$clicks_tbl.ip  ".$where;
        $sql.= " ORDER BY  $clicks_tbl.id DESC";
        $result = $db->get($sql);
                    ...
                    <table class="wp-list-table widefat fixed striped posts">
                        ...
                        <tbody id="the-list">
                            ...
                            <tr id="post-1" class="iedit author-self level-0 post-1 type-post status-publish format-standard hentry category-uncategorized">
                                ...
                                <td class="date column-date" data-colname="Date">
                                    <?php echo ads_mysql_timestamp_to_humen($print->clicked_time); ?>
                                </td>
                            </tr>
                            <?php } ?>
                            <?php if($count==0){ ?>
